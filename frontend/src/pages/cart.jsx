      ...
      <div>
        {
          cartData.map((item,index)=>{
            const productData=product.find((product0)=> product0._id===item._id);
            return (
      ...
      <div className="flex justify-end my-20">
        <div className="w-full sm:w-[450px]">
          ...
          <div className="w-full text-end">
            <button onClick={()=>navigate('/placeorder')}className="bg-black text-white text-sm my-7 px-7 py-3">PROCEED FOR PAYMENT</button>
          </div>
        </div>
      </div>
    </div>
